...
# File: label_make.py
# Update: 2020/05/26
# USAGE: python label_make.py
# Description: 
# 功能一：把标注工具得到的xml文件转为label.txt；功能二：得到train.txt。
# Annotations ---- 这个文件夹是放所有xml描述文件的。
# JPEGImages ---- 这个文件夹是放所有jpg图片文件的。
# ImageSets -> Main ---- 这个文件里稍后会生成一个名为train.txt的文档，train.txt文档内容是所有训练集图片的名字，没有后缀名。
########################################################################################################
...
#     if not os.path.exists('/media/lab/data//detection_data/Annotations/%s.xml'%(image_id)):
#         return
#     in_file = open('/media/lab/data//detection_data/Annotations/%s.xml'%(image_id))    #与图片对应的xml文件所在的地址
#     out_file = open('/home/lab/darknet/detection_data/labels/%s.txt'%(image_id), 'w')   #与此xml对应的转换后的txt，这个txt的保存完整路径
...
for year, image_set in sets:
    if not os.path.exists('/media/lab/data/detection_data/labels/'):
        os.makedirs('/media/lab/data/detection_data/labels/')
    image_ids = open('/media/lab/data/detection_data/name_list.txt').read().strip().split()
    list_file = open('/media/lab/data/detection_data/train.txt', 'w') #把结果写入到train.txt文件中，如果是训练集数据打开这一行，注释下一行
    # list_file = open('/media/lab/data/detection_data/val.txt', 'w')     #把结果写入到val.txt文件中，如果是验证数据集数据打开这一行，注释上一行
    for image_id in image_ids:
        print(image_id)
        list_file.write('/media/lab/data/detection_data/JPEGImages/' + image_id + '\n')   #把每一用于训练或验证的图片的完整的路径写入到infrared_train.txt中  这个文件会被voc.data yolo.c调用
        # convert_annotation(year, image_id)   #把图片的名称id传给函数，用于把此图片对应的xml中的数据转换成yolo要求的txt格式
    list_file.close()
